...
import { dhikrList } from "../data/dhikrList";
...
const Dhikr = () => {
  ...
  const getDisplayText = (dhikr) => {
    if (language === "ar") {
      return dhikr.arabic; // Show Arabic text when Arabic is selected
    }
    return dhikr.transliteration; // Show English transliteration when English is selected
  };
  ...
  return (
    <div
      className="bg-gradient-to-r from-gold to-gold/20 rounded-xl shadow-xl hover:shadow-2xl hover:scale-101 transition-all duration-300 ease-out p-4 sm:p-5 md:p-6 lg:p-7 xl:p-8 2xl:p-10 w-full h-full flex flex-col cursor-pointer relative overflow-hidden"
      ...
      <div className="flex flex-col h-full justify-between relative z-10">
        ...
        <div className="flex justify-between items-start flex-shrink-0">
          ...
          <div className="flex items-center space-x-1 sm:space-x-1.5 md:space-x-2 lg:space-x-2.5 xl:space-x-3 flex-shrink-0">
            ...
            <Tooltip content="Reset Dhikr count" position="left">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setDhikrCount(0);
